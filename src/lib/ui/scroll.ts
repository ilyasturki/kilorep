/**
 * Bringing an element back on screen, but only when it actually left.
 *
 * The workout pane pulls the active set to centre; before this file it did so
 * unconditionally, which meant tapping a row already on screen still slid the
 * whole page underneath the thumb. The visibility test is the fix, and it
 * lives beside the scroll it gates so no caller can take one without the
 * other.
 */
...
/**
 * Centre `node` in its scroller — unless every pixel of it is already on
 * screen, in which case the page holds still.
 *
 ...
 */
export function revealCentered(node: HTMLElement): void {
