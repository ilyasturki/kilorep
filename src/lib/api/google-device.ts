...
// `crypto.subtle` needs a secure context, which the WebView only has because
// `androidScheme` stays `https` (capacitor.config.ts).
...
// `browserFinished` can arrive either side of `appUrlOpen` on a sign-in that
// worked, so a close only counts as cancellation after this grace.
