...
{#if restTimer.running || restTimer.undoing}
	...
	<div class="vt-tabbar shrink-0 lg:px-3 lg:pb-3">
		{#if !restTimer.running}
			<!-- The five seconds after SKIP: the strip holds its place, drops the
			     clock and everything that steered it, and offers the one thing a
			     skip can still need. No digits, because nothing is counting — a
			     stopped clock parked above the tab bar reads as a bug, and the
			     strip is an aftermath, not a paused rest.

			     `role="status"` so the offer is announced rather than being a
			     control that appears silently and is gone in five seconds. -->
			<div
				role="status"
			...
			>
				...
				<!-- Where SKIP stood, wearing what SKIP wore: the thumb that just
				     pressed there is the thumb that wants this, and moving the target
				     would cost the recovery the five seconds are for. -->
				<button
					type="button"
		...
		{:else}
			...
			>
				...
				<!-- Not a `Button`: every variant that component offers is a commit, a
				     ...
				     `skip` and not `clear`: the difference is the five seconds of undo
				     that follow, which a set logged onto a warmup must not get. -->
				<button
					type="button"
				...
				<!-- Mute is the session's off switch, not the app's — PRODUCT.md wants
				     ...
				     the circuit day: gone at FINISH, editing nothing. It stays final:
				     `mute` clears rather than skips, so the bell leaves no undo behind. -->
				<button
					type="button"
