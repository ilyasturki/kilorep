...
- Data-out: CSV export, self-serve. No *public* API — but the teardown shows a private REST surface behind it (`/api/v3/export`, `/set`, `/user`, `/sharing`, `/track`, plus `/api/tpg/v7/plan` for plan generation), so the capability exists and is simply not offered to users. No history import; only plan-sharing via QR/link.
...
- Platform: iOS + Android, no web, no watch app. Confirmed 2026-07-25: no app subdomain resolves, `/app` redirects to a `download-app` store funnel, and the web bundle is not served anywhere on the domain (paths under `/assets/` return an empty 200 for any filename, real or invented). Yet per Stack below the app *is* a web app — so they are sitting on a browser build they decline to ship. The missing web surface is a deliberate distribution choice, not a technical limit, which is exactly the gap kilorep's criterion 3 attacks.
- Stack: a **Capacitor-wrapped web app**, and the only competitor stack verified by teardown rather than desk research. No cross-platform runtime: the arm64 split ships exactly one native lib (`libsigner.so`, Adjust attribution) — no `libflutter.so`, no Hermes/JSC. No UI framework either: zero trace of React, Vue, Angular, Svelte, Solid, Preact or Ionic components. It is hand-rolled vanilla JS + **jQuery 3.7.1**, with handlers declared inline as `data-data='{"click":{"action":"activateTabSection","pageId":"Training"}}'` and the tab bar as static markup in `index.html`. **PouchDB 9.0.0** on IndexedDB is the local store, replicating to **IBM Cloudant** (managed CouchDB — the bundle hard-codes a `*-bluemix.cloudant.com` endpoint). That CouchDB replication protocol *is* the offline story above, and it is the most directly reusable idea in the teardown. Chart.js 2.9.3, moment.js, Vite build, bundle string-array obfuscated. 169 KB of hand-written CSS: no Tailwind, 11 custom properties, 52 `@keyframes`. Type is **SF Pro Rounded** — Apple's rounded system face, five weights as ~1.4 MB `.otf` each, set on `body,button,input`; **Rubik** appears only as bold-italic accent type on the tagline and ProVersion paywall screens. Note they ship Apple's proprietary font inside an Android APK, which its license does not permit — do not copy that part. Firebase (auth/functions/storage/analytics/remote-config), RevenueCat for subscriptions, `cordova-plugin-health`. The native feel is bought with plugins — safe-area, navigation-bar, haptics, keep-awake (rest timer), local-notifications, date-picker, in-app-review. Ships ~3,500 asset files with one lazy chunk per exercise, hence a 166 MB download.
