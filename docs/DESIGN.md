...
- **A single lime accent.** Tailwind `lime-400` (`#a3e635`). v1's `#c5f53a` is retired in favour of palette consistency: the app takes its colours from Tailwind's ramp, which hands over `lime-300/500/600` for hover, pressed and disabled states for free. The two greens are close but not equal — v1's is lighter (L .905 vs .849) and more yellow (H 123.6 vs 128.9). One accent, in both themes, unchanged between them.
