...
- **`ssr = false` and `prerender = false` for the whole app, permanently.** One stray `+page.server.js` breaks the APK build, and not loudly. The directives live on `src/routes/(app)/+layout.ts`, not the root layout, because the root now also carries the marketing page. Two consequences, both accepted: every app route must go inside the `(app)` group, and the dev styleguide keeps its own `dev/+layout.ts` saying the same thing rather than inheriting app chrome. There is no longer a global net — a route added outside those two trees gets SvelteKit's default `ssr = true`.
- **The marketing page at `/` is the single exception**, and it is scoped to one file: `src/routes/+page.ts` sets `ssr = true` and `prerender = true`. Page options override the layout's, so the exception cannot leak sideways. It exists because the page is read by crawlers and link unfurlers, which do not run JavaScript — prerendering under `ssr = false` would emit an empty shell and be worth nothing.
...
- **The Capacitor entry must be `/start`, not `/`.** The marketing page owns `/`, so the APK currently boots into a pitch. `adapter-static` writes the prerendered pages and *then* overwrites `index.html` with the SPA fallback, warning `Overwriting build/app/index.html with fallback page`. So `build:app` stays green and the bundled `index.html` is the shell, not the pitch — but the shell then hydrates and its router resolves `/` to the landing route anyway. Deferred deliberately: Capacitor is not installed yet, so any fix written today would key off nothing and be rewritten. Solve it when the shell lands, either with a start path or a native-platform redirect. Blocks the first APK, nothing before it.
