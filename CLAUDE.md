...
Svelte 5 (runes) · Bits UI · Capacitor · in-memory domain over IndexedDB · SQLite + Drizzle over `node:sqlite` · `vitest`.

The test runner is vitest, not `bun test`: the server's driver is the `node:sqlite` builtin, which does not exist under Bun, so `bun test` cannot run server code at all. One runner, under Node, touching real SQLite.
...
Lives entirely in `src/lib/server/` (`db/`, `auth/`), with endpoints in `src/routes/api/**/+server.ts`. SvelteKit refuses to build if client code imports `$lib/server`, which is the point: it turns hard rule 4's failure mode — fine in Chrome, 404 in the APK — into a build error on the desktop.

- `bun run db:generate` after any schema edit, and **commit the SQL**. `hooks.server.ts` applies migrations at boot, so a self-hoster's upgrade is `docker run` and nothing else. The `drizzle/` folder must ship in the image.
- `bun run db:seed` is **development only** — the credentials are public, in the repository. Production bootstrap is an open question in [STACK.md](docs/STACK.md#open); do not quietly promote the seed script into the answer.
- `bun run build:server` → `build/server` (adapter-node). `bun run build:app` → `build/app` (adapter-static, the Capacitor bundle). Both adapters default to `build/`, so they are given separate directories on purpose.
- **Drizzle is pinned to `1.0.0-rc.4`, exactly, orm and kit together.** A mismatched pair fails with `SQLiteSyncDialect is not a constructor`. Do not widen either to a caret range.
...
- **Ids are uuids and timestamps are epoch milliseconds, everywhere.** Records are born offline on a phone, so ids cannot come from the server; milliseconds because two edits in the same second are exactly the conflict last-write-wins has to settle.
